...
// Calcola la distanza tra due boid
...
// Applica le regole dei Boid con OpenMP e ottimizzazioni
double applyRules(std::vector<Boid>& boids, int choice, std::vector<sf::VertexArray>& vectors) {
    auto start = std::chrono::high_resolution_clock::now();

    // Parallelizzazione del ciclo principale con OpenMP
#pragma omp parallel for default(none) shared(boids, vectors, choice) 
    for (int i = 0; i < boids.size(); ++i) {
        ...
        // Ciclo interno senza parallelizzazione
        for (int j = 0; j < boids.size(); ++j) {
            if (i != j) {
                float dist = distance(boids[i], boids[j]);
                if (dist < 100.0f) {
        ...
        // Protezione critica per l'accesso al vettore condiviso
#pragma omp critical
...
float calculateAverageSpeed(const std::vector<Boid>& boids) {
    float totalSpeed = 0.0f;
#pragma omp parallel for reduction(+:totalSpeed)
    for (int i = 0; i < boids.size(); ++i) {
        totalSpeed += std::sqrt(boids[i].velocity.x * boids[i].velocity.x + boids[i].velocity.y * boids[i].velocity.y);
    }
    return totalSpeed / boids.size();
}

int main() {
    sf::RenderWindow window(sf::VideoMode(800, 600), "Boids Simulation OpenMP");
    ...
    double applyRulesTimeSingolo = applyRules(boids, choice, vectors);
    ...
    while (window.isOpen()) {
        ...
        vectors.clear();
        double applyRulesTime = applyRules(boids, choice, vectors);
...
#pragma omp parallel for
        ...
        sf::Text textAvgSpeed("Velocità Media: " + std::to_string(calculateAverageSpeed(boids)), font, 20);
